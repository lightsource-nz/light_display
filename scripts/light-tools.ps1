...
# CMake already uses (CMakeLists.txt reads it from the environment before anything else). But a
# fresh shell may not have it set, and failing with "cannot find light-build.ps1" would send you
# looking in the wrong place -- so fall back to the sibling checkout, which is the same default
# CMakeLists.txt uses, and say clearly when neither works.
